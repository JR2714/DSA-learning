...
> 2026-08-03 LeetCode 21 复盘补充：核心规则只有一条——**对象要活得比"创建它的函数"久吗？**

- 需要 → `new`（堆分配，生命周期由你控制，直到 `delete`）
- 不需要 → 栈上/局部变量，函数结束自动销毁

链表语境清单：

| 场景 | 要 new 吗 | 原因 |
|------|----------|------|
| 返回新构建的链表（合并、两数相加） | 节点要 new | 新数据要活过函数 |
| 复用输入节点（原地合并、反转） | 不需要 | 节点已存在，只改指针 |
| 哨兵随结果返回 | 要 new | 要活过函数 |
| 哨兵不随结果返回 | 栈上即可 | 函数结束就没用了 |

要点（配对/数组规则见上文 `new[]`/`delete[]` 小节，不重复）：

- 官方题解 `preHead = new ListNode(-1)` 返回的是 `preHead->next`，哨兵本身不随结果返回 → **可以放栈上**：`ListNode preHead(-1); ListNode* prev = &preHead;`，省一次堆分配 + 免泄漏（官方版 `new` 后不 `delete` 严格说是小泄漏）
- `buildList` 里的 `ListNode dummy;` 就是栈上哨兵的例子
- 数组场景判断链：大小编译期已知且不大 → 栈上；运行期才知道或很大 → `std::vector`；必须裸指针才用 `new[]`
- 现代 C++ 原则：能用栈对象/标准容器就优先，它们把 `new`/`delete` 藏起来了
...
> 推荐习题清单：[Ch.1–2 习题推荐](../Exercise/chap1-2ex.md)
